...
//
// Usage: bun src/remotion/preview-all.ts
//
// Scans output/[name]/clip.json, generates a single TypeScript file that exports
// all clips, symlinks any missing videos into src/remotion/public/, then opens Studio.
...
if (import.meta.main) {
  ...
  await $`cd ${resolve(projectRoot, "src/remotion")} && bunx remotion studio index.tsx`.quiet();
}
