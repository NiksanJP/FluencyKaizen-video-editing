#!/usr/bin/env bun
/**
 * Studio server — serves the split-pane UI and bridges WebSockets to PTY processes.
 *
...
async function cleanupExistingClip(compId: string) {
  ...
  const clipSymlink = resolve(projectRoot, "src/remotion/clips", compId);
  try {
  ...
  async fetch(req, server) {
    ...
    if (path === "/api/import-video" && req.method === "POST") {
      ...
      try {
        ...
        try {
          pipelineProcess = Bun.spawn(["bun", "src/pipeline/index.ts", destPath, "--lang", targetLang], {
            cwd: projectRoot,
    ...
    if (clipDeleteMatch && req.method === "DELETE") {
      ...
      try {
        ...
        await rm(resolve(projectRoot, "src/remotion/clips", compId), {
          recursive: true,
          force: true,
        });
        await rm(resolve(projectRoot, "src/remotion/public", compId), {
          recursive: true,
          force: true,
        });
    ...
    if (clipApiMatch && req.method === "PUT") {
      ...
      try {
        ...
        try {
          const regen = Bun.spawn(["bun", resolve(projectRoot, "src/remotion/watch-clip.ts")], {
            cwd: projectRoot,
    ...
    // Serve studio styles.css at root (referenced by index.html)
    if (path === "/styles.css") {
      try {
        ...
        <style>
          ...
          *, *::before, *::after { box-sizing: inherit; }
          ...
          #__remotion-studio-container {
            ...
            top: 0 !important;
            left: 0 !important;
            right: var(--fk-assistant-panel-width) !important;
            bottom: 0 !important;
...
console.log(`Studio server running at http://localhost:${server.port}`);
