 ...
 *
 * Usage: bun src/studio/launch.ts [clip-name]
 */
...
async function ensureVideoSymlinksLocal(clips: Array<{ name: string; data: any; videoFile: string }>) {
  ...
  for (const clip of clips) {
    ...
    try {
      ...
      console.log(`  Copied ${clip.videoFile} → src/remotion/public/${clip.name}/`);
    } catch {
...
async function generateAllClipsFileLocal(clips: Array<{ name: string; data: any; videoFile: string }>) {
  const allClipsPath = resolve(projectRoot, "src/remotion/clip-data-all.ts");
    ...
    const settings = {
      permissions: {
        allow: [
          ...
          "Edit(./audio.json)",
          "Read(**)",
          `Bash(cd ${projectRoot}/src/remotion && bun remotion *)`,
          `Bash(bun ${projectRoot}/src/remotion/watch-clip.ts)`,
        ],
      },
    };
...
## Steps
1. Read \`./clip.json\`
2. Run: \`bun ${projectRoot}/src/remotion/watch-clip.ts\`
   Or manually regenerate clip-data-all.ts at \`${projectRoot}/src/remotion/clip-data-all.ts\`
`);
  }
...
if (clips.length === 0) {
  ...
  // Symlink TSX/style from output/ → src/remotion/clips/ (instant HMR)
  console.log("  Symlinking TSX files to src/remotion/clips/...");
  for (const clip of clips) {
    const tsxPath = resolve(outputDir, clip.name, "ClipComposition.tsx");
    if (existsSync(tsxPath)) {
...
const watcher = Bun.spawn(["bun", resolve(projectRoot, "src/remotion/watch-clip.ts")], {
  cwd: projectRoot,
...
// 3. Studio server (home page + split view + proxy)
console.log(`Starting studio server on port ${wrapperPort}...`);
const wrapperServer = Bun.spawn(["bun", resolve(projectRoot, "src/studio/server.ts")], {
  cwd: projectRoot,
  stdio: ["ignore", "inherit", "inherit"],
  env: {
    ...process.env,
...
if (!wrapperReady) {
  console.error("Studio server failed to start within 15 seconds.");
  cleanup();
  process.exit(1);
}
console.log("  Studio server is ready");
