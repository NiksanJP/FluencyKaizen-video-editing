import type { ClipData } from "../../pipeline/types";

const allClips: Record<string, ClipData> = {
  "001": {
  "videoFile": "001/clip_trimmed.mp4",
  "hookTitle": {
    "ja": "進捗報告",
    "en": "Progress Report Tips!"
  },
  "clip": {
    "startTime": 0,
    "endTime": 13.31
  },
  "subtitles": [
    {
      "startTime": 0,
      "endTime": 1.6799999999999997,
      "en": "Good morning, Nick.",
      "ja": "おはようございます、ニック。",
      "highlights": [
        "おはようございます"
      ],
      "enHighlights": [
        "Good morning"
      ]
    },
    {
      "startTime": 1.8599999999999994,
      "endTime": 3.08,
      "en": "We're here to discuss",
      "ja": "本日は、について",
      "highlights": [],
      "enHighlights": []
    },
    {
      "startTime": 3.1599999999999984,
      "endTime": 3.679999999999998,
      "en": "your progress",
      "ja": "進捗状況について",
      "highlights": [
        "進捗状況"
      ],
      "enHighlights": [
        "progress"
      ]
    },
    {
      "startTime": 3.839999999999998,
      "endTime": 5.26,
      "en": "market research analysis",
      "ja": "市場調査分析の",
      "highlights": [
        "市場調査分析"
      ],
      "enHighlights": [
        "market research analysis"
      ]
    },
    {
      "startTime": 5.499999999999998,
      "endTime": 6.360000000000001,
      "en": "for Project X.",
      "ja": "プロジェクトXの",
      "highlights": [
        "プロジェクトX"
      ],
      "enHighlights": [
        "Project X"
      ]
    },
    {
      "startTime": 6.9399999999999995,
      "endTime": 7.74,
      "en": "Please provide a",
      "ja": "ご報告をお願いします",
      "highlights": [
        "ご報告"
      ],
      "enHighlights": [
        "provide"
      ]
    },
    {
      "startTime": 7.700000000000001,
      "endTime": 8.339999999999998,
      "en": "concise update.",
      "ja": "簡潔な最新情報で。",
      "highlights": [
        "簡潔な",
        "最新情報"
      ],
      "enHighlights": [
        "concise",
        "update"
      ]
    },
    {
      "startTime": 9.22,
      "endTime": 10.26,
      "en": "Good morning, Mr. Tanaka.",
      "ja": "田中さん、おはようございます。",
      "highlights": [
        "おはようございます"
      ],
      "enHighlights": [
        "Good morning"
      ]
    },
    {
      "startTime": 10.38,
      "endTime": 10.94,
      "en": "How are you?",
      "ja": "お元気ですか？",
      "highlights": [],
      "enHighlights": []
    },
    {
      "startTime": 11.140000000000002,
      "endTime": 12.140000000000002,
      "en": "I'm well. Thank you.",
      "ja": "元気です。ありがとう。",
      "highlights": [],
      "enHighlights": []
    },
    {
      "startTime": 12.660000000000005,
      "endTime": 13.280000000000001,
      "en": "Let's proceed.",
      "ja": "では、始めましょう。",
      "highlights": [
        "始めましょう"
      ],
      "enHighlights": [
        "proceed"
      ]
    }
  ],
  "vocabCards": [
    {
      "triggerTime": 3.380000000000001,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "progress",
      "literal": "進捗",
      "nuance": "ビジネスの文脈では、プロジェクトやタスクの「進捗状況」を指す。報告会などで頻繁に使われる。「進捗はいかがですか？」と尋ねたり、「進捗を報告する」のように使う。"
    },
    {
      "triggerTime": 4.299999999999999,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "market research analysis",
      "literal": "市場調査分析",
      "nuance": "特定の製品やサービスについて、市場の状況を深く理解するための調査と、その結果を詳細に分析する作業。新規事業の立ち上げや既存事業の改善に不可欠なプロセス。"
    },
    {
      "triggerTime": 7.700000000000001,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "concise update",
      "literal": "簡潔な最新情報／報告",
      "nuance": "長くならず、要点を押さえて最新の状況を報告すること。特に、忙しい上司やクライアントへの報告時に求められるスキル。時間を効率的に使う意識が込められている。"
    },
    {
      "triggerTime": 11.040000000000001,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "proceed",
      "literal": "進む／続ける",
      "nuance": "会議や作業を「進める」「開始する」という意味で使われるフォーマルな表現。相手に次のステップへの移行を促す際や、自分が次の議題に進む際に「Let's proceed.」のように言う。"
    }
  ],
  "videoDuration": 13.31,
  "silenceGaps": [
    {
      "originalStart": 24.44,
      "originalEnd": 27.639999999999997,
      "duration": 3.1999999999999957
    }
  ]
} as unknown as ClipData,
  "005": {
  "videoFile": "005/clip_trimmed.mp4",
  "hookTitle": {
    "ja": "🅿️「Parking」って何？ビジネス英語の使い方",
    "en": "🅿️ \"Parking\" Projects..."
  },
  "clip": {
    "startTime": 0,
    "endTime": 88.189
  },
  "subtitles": [
    {
      "startTime": 0,
      "endTime": 1.4399999999999995,
      "en": "Nick, thank you for",
      "ja": "ニック、お時間を割いていただき",
      "highlights": [
        "お時間"
      ],
      "enHighlights": [
        "thank you for"
      ]
    },
    {
      "startTime": 1.2599999999999998,
      "endTime": 2.4399999999999995,
      "en": "scheduling this one...",
      "ja": "この1対1の面談を組んでくれて",
      "highlights": [
        "1対1の面談"
      ],
      "enHighlights": [
        "one on one"
      ]
    },
    {
      "startTime": 2.879999999999999,
      "endTime": 3.9399999999999977,
      "en": "I understand there are",
      "ja": "いくつかあると理解しています",
      "highlights": [
        "理解しています"
      ],
      "enHighlights": [
        "understand"
      ]
    },
    {
      "startTime": 3.7799999999999976,
      "endTime": 4.960000000000001,
      "en": "a few items we need",
      "ja": "いくつか議論すべき事項が",
      "highlights": [
        "議論すべき事項"
      ],
      "enHighlights": [
        "items we need"
      ]
    },
    {
      "startTime": 4.759999999999998,
      "endTime": 5.460000000000001,
      "en": "to discuss regarding",
      "ja": "～について話し合うべきです",
      "highlights": [
        "話し合うべき"
      ],
      "enHighlights": [
        "discuss regarding"
      ]
    },
    {
      "startTime": 5.640000000000001,
      "endTime": 6.84,
      "en": "current priorities and",
      "ja": "現在の優先順位や",
      "highlights": [
        "優先順位"
      ],
      "enHighlights": [
        "priorities"
      ]
    },
    {
      "startTime": 6.859999999999999,
      "endTime": 7.5,
      "en": "Project X.",
      "ja": "プロジェクトXについてです",
      "highlights": [
        "プロジェクトX"
      ],
      "enHighlights": [
        "Project X"
      ]
    },
    {
      "startTime": 7.939999999999998,
      "endTime": 8.779999999999998,
      "en": "Please proceed with",
      "ja": "進めてください",
      "highlights": [
        "進めてください"
      ],
      "enHighlights": [
        "proceed with"
      ]
    },
    {
      "startTime": 8.64,
      "endTime": 9.079999999999998,
      "en": "your updates.",
      "ja": "あなたの最新情報を",
      "highlights": [
        "最新情報"
      ],
      "enHighlights": [
        "updates"
      ]
    },
    {
      "startTime": 9.340000000000003,
      "endTime": 10.640000000000004,
      "en": "Hey, Mr. Tanaka,",
      "ja": "田中さん",
      "highlights": [
        "田中さん"
      ],
      "enHighlights": [
        "Mr. Tanaka"
      ]
    },
    {
      "startTime": 10.780000000000005,
      "endTime": 11.440000000000005,
      "en": "yeah, I'm actually",
      "ja": "ええ、実は",
      "highlights": [
        "実は"
      ],
      "enHighlights": [
        "actually"
      ]
    },
    {
      "startTime": 11.380000000000003,
      "endTime": 12.660000000000004,
      "en": "working on Project Y.",
      "ja": "プロジェクトYに取り組んでいます",
      "highlights": [
        "プロジェクトY"
      ],
      "enHighlights": [
        "Project Y"
      ]
    },
    {
      "startTime": 13.040000000000003,
      "endTime": 13.840000000000003,
      "en": "I'm actually parking",
      "ja": "実は保留しています",
      "highlights": [
        "保留しています"
      ],
      "enHighlights": [
        "parking"
      ]
    },
    {
      "startTime": 14.060000000000006,
      "endTime": 15.520000000000003,
      "en": "Project X for now",
      "ja": "プロジェクトXは一時的に",
      "highlights": [
        "一時的に"
      ],
      "enHighlights": [
        "for now"
      ]
    },
    {
      "startTime": 15.420000000000005,
      "endTime": 16.300000000000004,
      "en": "due to stakeholder",
      "ja": "ステークホルダーの",
      "highlights": [
        "ステークホルダー"
      ],
      "enHighlights": [
        "stakeholder"
      ]
    },
    {
      "startTime": 16.620000000000005,
      "endTime": 16.920000000000005,
      "en": "reprioritization.",
      "ja": "優先順位の変更のためです",
      "highlights": [
        "優先順位の変更"
      ],
      "enHighlights": [
        "reprioritization"
      ]
    },
    {
      "startTime": 18.320000000000007,
      "endTime": 19.56000000000001,
      "en": "I'll get back to this",
      "ja": "これには後で戻ります",
      "highlights": [
        "後で戻ります"
      ],
      "enHighlights": [
        "get back to"
      ]
    },
    {
      "startTime": 19.540000000000006,
      "endTime": 20.62000000000001,
      "en": "when I have",
      "ja": "時間があるときに",
      "highlights": [
        "時間があるときに"
      ],
      "enHighlights": [
        "when I have"
      ]
    },
    {
      "startTime": 20.860000000000007,
      "endTime": 22.540000000000006,
      "en": "parking project X.",
      "ja": "プロジェクトXを保留にしています",
      "highlights": [
        "保留にしています"
      ],
      "enHighlights": [
        "parking project X"
      ]
    },
    {
      "startTime": 23.440000000000012,
      "endTime": 23.74000000000001,
      "en": "I see.",
      "ja": "なるほど。",
      "highlights": [],
      "enHighlights": []
    },
    {
      "startTime": 23.98000000000001,
      "endTime": 24.88000000000001,
      "en": "Could you elaborate on",
      "ja": "詳しく説明してもらえますか",
      "highlights": [
        "詳しく説明"
      ],
      "enHighlights": [
        "elaborate on"
      ]
    },
    {
      "startTime": 24.92000000000001,
      "endTime": 25.38000000000001,
      "en": "the stakeholder",
      "ja": "ステークホルダーの",
      "highlights": [
        "ステークホルダー"
      ],
      "enHighlights": [
        "stakeholder"
      ]
    },
    {
      "startTime": 25.56000000000001,
      "endTime": 25.860000000000007,
      "en": "reprioritization",
      "ja": "優先順位の変更について",
      "highlights": [
        "優先順位の変更"
      ],
      "enHighlights": [
        "reprioritization"
      ]
    },
    {
      "startTime": 26.680000000000007,
      "endTime": 27.820000000000007,
      "en": "that necessitates this",
      "ja": "これを必要とする",
      "highlights": [
        "必要とする"
      ],
      "enHighlights": [
        "necessitates"
      ]
    },
    {
      "startTime": 27.820000000000007,
      "endTime": 28.12000000000001,
      "en": "hold?",
      "ja": "保留は？",
      "highlights": [
        "保留"
      ],
      "enHighlights": [
        "hold"
      ]
    },
    {
      "startTime": 28.70000000000001,
      "endTime": 29.600000000000012,
      "en": "It's critical we",
      "ja": "私たちは重要だと",
      "highlights": [
        "重要"
      ],
      "enHighlights": [
        "critical"
      ]
    },
    {
      "startTime": 29.46000000000001,
      "endTime": 30.580000000000002,
      "en": "understand the impact on",
      "ja": "影響を理解することが",
      "highlights": [
        "影響を理解"
      ],
      "enHighlights": [
        "understand the impact"
      ]
    },
    {
      "startTime": 30.500000000000004,
      "endTime": 31.480000000000008,
      "en": "the Project X timeline.",
      "ja": "プロジェクトXのタイムラインに",
      "highlights": [
        "プロジェクトX",
        "タイムライン"
      ],
      "enHighlights": [
        "Project X",
        "timeline"
      ]
    },
    {
      "startTime": 32.16000000000001,
      "endTime": 33.60000000000001,
      "en": "Yeah, as you can imagine",
      "ja": "ええ、ご想像の通り",
      "highlights": [
        "ご想像の通り"
      ],
      "enHighlights": [
        "as you can imagine"
      ]
    },
    {
      "startTime": 34.08,
      "endTime": 35.7,
      "en": "how busy I am,",
      "ja": "私がどれだけ忙しいか",
      "highlights": [
        "忙しい"
      ],
      "enHighlights": [
        "busy I am"
      ]
    },
    {
      "startTime": 36.000000000000014,
      "endTime": 36.800000000000026,
      "en": "the stakeholder came",
      "ja": "ステークホルダーが来て",
      "highlights": [
        "ステークホルダー"
      ],
      "enHighlights": [
        "stakeholder"
      ]
    },
    {
      "startTime": 37.10000000000002,
      "endTime": 38.24000000000002,
      "en": "and told me in public",
      "ja": "そして公の場で私に",
      "highlights": [
        "公の場"
      ],
      "enHighlights": [
        "in public"
      ]
    },
    {
      "startTime": 37.800000000000026,
      "endTime": 38.96000000000002,
      "en": "public channels that",
      "ja": "公のチャネルであなたが",
      "highlights": [
        "公のチャネル"
      ],
      "enHighlights": [
        "public channels"
      ]
    },
    {
      "startTime": 38.660000000000025,
      "endTime": 40.10000000000002,
      "en": "you are also in to",
      "ja": "あなたが〜に参加していると",
      "highlights": [
        "参加している"
      ],
      "enHighlights": [
        "are also in"
      ]
    },
    {
      "startTime": 40.26000000000002,
      "endTime": 41.76000000000002,
      "en": "reprioritize this task.",
      "ja": "このタスクを再優先するよう",
      "highlights": [
        "再優先する"
      ],
      "enHighlights": [
        "reprioritize this task"
      ]
    },
    {
      "startTime": 42.08000000000001,
      "endTime": 43.880000000000024,
      "en": "While I appreciate the",
      "ja": "私が感謝している間も",
      "highlights": [
        "感謝している"
      ],
      "enHighlights": [
        "appreciate"
      ]
    },
    {
      "startTime": 43.800000000000026,
      "endTime": 44.48000000000002,
      "en": "direct communication,",
      "ja": "直接のコミュニケーションはありがたいが",
      "highlights": [
        "直接のコミュニケーション"
      ],
      "enHighlights": [
        "direct communication"
      ]
    },
    {
      "startTime": 45.62000000000002,
      "endTime": 46.24000000000002,
      "en": "significant project",
      "ja": "重要なプロジェクトの",
      "highlights": [
        "重要なプロジェクト"
      ],
      "enHighlights": [
        "significant project"
      ]
    },
    {
      "startTime": 46.46000000000002,
      "endTime": 47.72000000000003,
      "en": "changes like this need",
      "ja": "このような変更には",
      "highlights": [
        "変更"
      ],
      "enHighlights": [
        "changes"
      ]
    },
    {
      "startTime": 47.86000000000003,
      "endTime": 48.54000000000002,
      "en": "formal documentation.",
      "ja": "正式な文書が必要です",
      "highlights": [
        "正式な文書"
      ],
      "enHighlights": [
        "formal documentation"
      ]
    },
    {
      "startTime": 49.50000000000003,
      "endTime": 50.46000000000002,
      "en": "Please send a formal",
      "ja": "正式なものを送ってください",
      "highlights": [
        "正式なもの"
      ],
      "enHighlights": [
        "formal"
      ]
    },
    {
      "startTime": 50.48000000000002,
      "endTime": 51.36000000000003,
      "en": "confirmation detailing",
      "ja": "詳細を記した確認書を",
      "highlights": [
        "確認書"
      ],
      "enHighlights": [
        "confirmation"
      ]
    },
    {
      "startTime": 51.64000000000003,
      "endTime": 52.46000000000002,
      "en": "the new priority",
      "ja": "新しい優先順位の",
      "highlights": [
        "優先順位"
      ],
      "enHighlights": [
        "priority"
      ]
    },
    {
      "startTime": 52.58000000000003,
      "endTime": 54.04000000000002,
      "en": "structure and its impact",
      "ja": "構造とその影響を",
      "highlights": [
        "構造",
        "影響"
      ],
      "enHighlights": [
        "structure",
        "impact"
      ]
    },
    {
      "startTime": 54.08000000000003,
      "endTime": 54.920000000000016,
      "en": "on Project X.",
      "ja": "プロジェクトXへの",
      "highlights": [
        "プロジェクトX"
      ],
      "enHighlights": [
        "Project X"
      ]
    },
    {
      "startTime": 55.34000000000002,
      "endTime": 56.64000000000003,
      "en": "This is essential for",
      "ja": "これは不可欠です",
      "highlights": [
        "不可欠"
      ],
      "enHighlights": [
        "essential"
      ]
    },
    {
      "startTime": 56.58000000000003,
      "endTime": 57.34000000000002,
      "en": "portfolio management.",
      "ja": "ポートフォリオ管理のために",
      "highlights": [
        "ポートフォリオ管理"
      ],
      "enHighlights": [
        "portfolio management"
      ]
    },
    {
      "startTime": 58.12000000000002,
      "endTime": 60.04000000000002,
      "en": "Okay, I'll just let you",
      "ja": "分かりました、ただあなたに",
      "highlights": [
        "分かりました"
      ],
      "enHighlights": [
        "Okay"
      ]
    },
    {
      "startTime": 59.920000000000016,
      "endTime": 61.54000000000002,
      "en": "know I'm putting this",
      "ja": "このプロジェクトを",
      "highlights": [
        "プロジェクト"
      ],
      "enHighlights": [
        "this project"
      ]
    },
    {
      "startTime": 61.48000000000002,
      "endTime": 62.84000000000002,
      "en": "project to hold for now.",
      "ja": "一時的に保留にします",
      "highlights": [
        "一時的に保留"
      ],
      "enHighlights": [
        "hold for now"
      ]
    },
    {
      "startTime": 63.10000000000001,
      "endTime": 64.06,
      "en": "Understood, Nick.",
      "ja": "承知しました、ニック",
      "highlights": [
        "承知しました"
      ],
      "enHighlights": [
        "Understood"
      ]
    },
    {
      "startTime": 64.46000000000001,
      "endTime": 65.64,
      "en": "Just ensure the formal",
      "ja": "正式な連絡を確実に",
      "highlights": [
        "正式な連絡"
      ],
      "enHighlights": [
        "formal"
      ]
    },
    {
      "startTime": 65.66000000000001,
      "endTime": 67,
      "en": "communication is sent...",
      "ja": "早めに送ってください",
      "highlights": [
        "早めに送って"
      ],
      "enHighlights": [
        "sent soon"
      ]
    },
    {
      "startTime": 67.06,
      "endTime": 68.3,
      "en": "for proper record...",
      "ja": "適切な記録のために",
      "highlights": [
        "適切な記録"
      ],
      "enHighlights": [
        "proper record keeping"
      ]
    },
    {
      "startTime": 68.8,
      "endTime": 69.66000000000001,
      "en": "Thank you for the update.",
      "ja": "最新情報をありがとうございます",
      "highlights": [
        "最新情報"
      ],
      "enHighlights": [
        "the update"
      ]
    },
    {
      "startTime": 70.36,
      "endTime": 71.64,
      "en": "No worries, thank you.",
      "ja": "どういたしまして、ありがとう",
      "highlights": [],
      "enHighlights": []
    },
    {
      "startTime": 71.82,
      "endTime": 73.6,
      "en": "To summarize, we are",
      "ja": "要約すると、私たちは",
      "highlights": [
        "要約すると"
      ],
      "enHighlights": [
        "To summarize"
      ]
    },
    {
      "startTime": 73.44,
      "endTime": 74.6,
      "en": "parking Project X",
      "ja": "プロジェクトXを保留にしています",
      "highlights": [
        "プロジェクトX",
        "保留"
      ],
      "enHighlights": [
        "parking Project X"
      ]
    },
    {
      "startTime": 74.64,
      "endTime": 75.46,
      "en": "for the time being",
      "ja": "当面の間",
      "highlights": [
        "当面の間"
      ],
      "enHighlights": [
        "for the time being"
      ]
    },
    {
      "startTime": 76.67999999999999,
      "endTime": 77.74,
      "en": "based on stakeholder",
      "ja": "ステークホルダーの",
      "highlights": [
        "ステークホルダー"
      ],
      "enHighlights": [
        "stakeholder"
      ]
    },
    {
      "startTime": 77.86,
      "endTime": 78.16,
      "en": "reprioritization.",
      "ja": "優先順位変更に基づき",
      "highlights": [
        "優先順位変更"
      ],
      "enHighlights": [
        "reprioritization"
      ]
    },
    {
      "startTime": 79.36,
      "endTime": 80.83999999999999,
      "en": "It is crucial that you",
      "ja": "あなたが送ることが重要です",
      "highlights": [
        "重要"
      ],
      "enHighlights": [
        "crucial"
      ]
    },
    {
      "startTime": 80.69999999999999,
      "endTime": 81.36,
      "en": "send the formal",
      "ja": "正式な",
      "highlights": [
        "正式な"
      ],
      "enHighlights": [
        "formal"
      ]
    },
    {
      "startTime": 81.33999999999999,
      "endTime": 82.52,
      "en": "confirmation to ensure",
      "ja": "確認書を確実に送ることです",
      "highlights": [
        "確認書",
        "確実に"
      ],
      "enHighlights": [
        "confirmation",
        "ensure"
      ]
    },
    {
      "startTime": 82.52,
      "endTime": 83.58,
      "en": "proper alignment and",
      "ja": "適切な整合性と",
      "highlights": [
        "適切な整合性"
      ],
      "enHighlights": [
        "proper alignment"
      ]
    },
    {
      "startTime": 83.72,
      "endTime": 84.3,
      "en": "record keeping.",
      "ja": "記録保持のために",
      "highlights": [
        "記録保持"
      ],
      "enHighlights": [
        "record keeping"
      ]
    },
    {
      "startTime": 84.82,
      "endTime": 85.66,
      "en": "Thank you for the update,",
      "ja": "最新情報をありがとう",
      "highlights": [
        "最新情報"
      ],
      "enHighlights": [
        "the update"
      ]
    },
    {
      "startTime": 85.6,
      "endTime": 85.89999999999999,
      "en": "Nick.",
      "ja": "ニック",
      "highlights": [],
      "enHighlights": []
    },
    {
      "startTime": 86.24,
      "endTime": 86.97999999999999,
      "en": "Have a productive day.",
      "ja": "良い一日を",
      "highlights": [
        "良い一日"
      ],
      "enHighlights": [
        "productive day"
      ]
    },
    {
      "startTime": 87.78,
      "endTime": 88.08,
      "en": "Goodbye.",
      "ja": "さようなら",
      "highlights": [],
      "enHighlights": []
    }
  ],
  "vocabCards": [
    {
      "triggerTime": 1.8599999999999994,
      "duration": 3.5,
      "category": "社内英語",
      "phrase": "one on one",
      "literal": "1対1",
      "nuance": "「1対1の面談や会議」を意味するビジネス英語表現。特に上司と部下の定期的な面談で使われることが多い。"
    },
    {
      "triggerTime": 13.540000000000003,
      "duration": 3.5,
      "category": "社内英語",
      "phrase": "parking Project X",
      "literal": "プロジェクトXを駐車している",
      "nuance": "「プロジェクトXを一時的に保留にする、中断する」という意味のビジネス英語表現。車を「駐車する」ように、一時的にプロジェクトを「脇に置く」ニュアンス。"
    },
    {
      "triggerTime": 16.000000000000004,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "stakeholder reprioritization",
      "literal": "ステークホルダーの再優先順位付け",
      "nuance": "顧客、従業員、投資家などの「利害関係者 (stakeholder)」によって、プロジェクトやタスクの「優先順位が見直されること (reprioritization)」。ビジネスにおいて、外部からの要請で計画が変更される際に用いられる。"
    },
    {
      "triggerTime": 47.86000000000003,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "formal documentation",
      "literal": "正式な文書",
      "nuance": "口頭ではなく、書面として作成され、記録として残る公式な文書のこと。プロジェクトの変更や重要な決定事項には、この「正式な文書」が求められる。"
    },
    {
      "triggerTime": 67.72,
      "duration": 3.5,
      "category": "ビジネス英語",
      "phrase": "record keeping",
      "literal": "記録保持",
      "nuance": "組織やプロジェクトの情報を整理し、保存するプロセス。後の参照や監査のために重要であり、ビジネス運営の基本となる。"
    }
  ],
  "videoDuration": 88.189,
  "silenceGaps": [
    {
      "originalStart": 22.540000000000003,
      "originalEnd": 23.939999999999998,
      "duration": 1.399999999999995
    },
    {
      "originalStart": 32.92,
      "originalEnd": 33.98,
      "duration": 1.0599999999999952
    },
    {
      "originalStart": 39.64,
      "originalEnd": 40.28,
      "duration": 0.6400000000000006
    },
    {
      "originalStart": 52.300000000000004,
      "originalEnd": 53.019999999999996,
      "duration": 0.7199999999999918
    },
    {
      "originalStart": 59.1,
      "originalEnd": 61.879999999999995,
      "duration": 2.779999999999994
    },
    {
      "originalStart": 82.89999999999999,
      "originalEnd": 86.52000000000001,
      "duration": 3.6200000000000188
    },
    {
      "originalStart": 95.24,
      "originalEnd": 100.34,
      "duration": 5.1000000000000085
    }
  ]
} as unknown as ClipData
};

export default allClips;
