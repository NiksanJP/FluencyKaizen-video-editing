 ...
 *
 * TSX + style.json: symlinked from remotion/src/clips/[name]/ → output/[name]/
...
// --- Symlink sync: output/[name]/ ← remotion/src/clips/[name]/ ---

/** Create a symlink from remotion/src/clips/[name]/[relPath] → output/[name]/[relPath] */
...
export async function syncClipFiles(clipName: string) {
  ...
  console.log(`🔗 [${new Date().toLocaleTimeString()}] Symlinked ${clipName}/ → remotion/src/clips/${clipName}/`);
}
