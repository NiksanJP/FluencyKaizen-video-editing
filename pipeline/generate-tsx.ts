 ...
 *
 * Reads shared components from remotion/src/, rewrites import paths so they
 * resolve correctly when synced to remotion/src/clips/[name]/, and writes
 ...
 * are symlinked into remotion/src/clips/[name]/, webpack sees them at
...
function rewriteCompositionImports(source: string): string {
  return source
    // ../../pipeline/types → ../../pipeline/types  (already correct from output/[name]/)
    ...
    // ./clip-data-all → ../../remotion/src/clip-data-all
    .replace(
      /from\s+["']\.\/clip-data-all["']/g,
      'from "../../remotion/src/clip-data-all"'
    );
}
...
function rewriteComponentImports(source: string): string {
  return source
    // ../../../pipeline/types → ../../../pipeline/types  (already correct from output/[name]/components/)
    // ../../../style.json → ../style.json  (local copy in output/[name]/)
    .replace(
      /from\s+["']\.\.\/\.\.\/\.\.\/style\.json["']/g,
      'from "../style.json"'
...
export async function generateClipTsx(
  ...
  const remotionSrc = resolve(projectRoot, "remotion/src");
...
// CLI mode: bun pipeline/generate-tsx.ts <clipName>
if (import.meta.main) {
  const clipName = process.argv[2];
  if (!clipName) {
    console.error("Usage: bun pipeline/generate-tsx.ts <clip-name>");
    process.exit(1);
  }
